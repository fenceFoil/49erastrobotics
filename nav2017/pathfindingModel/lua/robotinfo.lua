...
function robotinfo.getCorners(botX, botY, botAngle)
  ...
  -- use theta1 to calculate all corners
  -- simultaneously add botAngle to rotate robot
  local thetas = {robotinfo.theta1 + botAngle, math.pi - robotinfo.theta1 + botAngle, math.pi + robotinfo.theta1 + botAngle, 2*math.pi - robotinfo.theta1 + botAngle}
  
  -- convert corner points from polar to x,y, simultaneously adding botX and botY
  local xycoords = {}
  for i,theta in ipairs(thetas) do
    -- x
    xycoords[#xycoords+1] = robotinfo.r*math.cos(theta) + botX

    -- y
    xycoords[#xycoords+1] = robotinfo.r*math.sin(theta) + botY
  end
